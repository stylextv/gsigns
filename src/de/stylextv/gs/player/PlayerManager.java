...
public class PlayerManager {
	...
	public static UUID placeSign(Order order, Location first, Location second) {
		if(first.getWorld().equals(second.getWorld())) {
				...
				if(valid) {
					...
					if(save) {
						...
						if(dir.getX()!=0) {
							...
							if(WorldUtil.getMcVersion()<WorldUtil.MCVERSION_1_13) {
								throw new InvalidParameterException("Item frames can only be placed on the §cfloor/ceiling§7 when using minecraft version §c1.13§7 or higher.");
							}
			...
			if(!placed) {
				throw new InvalidParameterException("No valid location found.");
			}
		} else {
			throw new InvalidParameterException("The two corners have to be in the same world.");
		}
		return null;
	}
