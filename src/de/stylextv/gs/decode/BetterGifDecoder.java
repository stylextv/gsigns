 ...
 *
 * <pre>
 * final GifImage gifImage = BetterGifDecoder.read(int[] data);
...
public final class BetterGifDecoder {
	...
	public final class GifImage {
		...
		/**
		 * If not 0, the delay specifies how many milliseconds
		 * to wait before displaying the frame <i>after</i> the current frame.
		 *
		 * @param index
		 *            Index of the current frame, 0 to N-1
		 * @return Delay as number of milliseconds
		 */
	...
	public static final GifImage read(final byte[] in) throws IOException {
		final BetterGifDecoder decoder = new BetterGifDecoder();
